#############################################################################################
# Plastic Bag Ban
# Small cell-level code, month level final data combination 
...
load('data/processed/00_data_intermediate/00_data_smallcell_month.rda')
...
data <- left_join(cleanupSmallCellMonth, merged %>% dplyr::select(zip, state, medianIncome, incQuartile, incQuintile, firstEffect, completeBagBan, partialBagBan, bagLimit, plasticBagCharge, paperBagCharge, paperBagBan, repealed, repealedDate, 
...
dataRep <- data %>% mutate(ind = 1) %>% group_by(groupID001, zip) %>% summarise(sum = sum(ind)) %>% ungroup()
dataRep <- dataRep%>% mutate(sum = 1) %>% group_by(groupID001) %>% summarise(sum = sum(sum)) %>% ungroup()
...
dataMultiple <- dataMultiple %>% arrange(groupID001, firstEffect)
dataMultiple <- dataMultiple %>% group_by(groupID001) %>% mutate(zip = first(zip), county = first(county), firstEffect = first(firstEffect),
...
data <- data %>% arrange(groupID001, year, month)
...
save(data, file="data/processed/02_data_merged/02_merged_smallcell_month.rda")
