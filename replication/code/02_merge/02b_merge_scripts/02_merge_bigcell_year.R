#############################################################################################
# Plastic Bag Ban
# Big cell code, year level final data combination 
...
data <- left_join(cleanupBigCellYear, merged %>% dplyr::select(zip, state, medianIncome, incQuartile, incQuintile, firstEffect, completeBagBan, partialBagBan, bagLimit, plasticBagCharge, paperBagCharge, paperBagBan, repealed, repealedDate, 
...
dataRep <- data %>% mutate(ind = 1) %>% group_by(groupID1, zip) %>% summarise(sum = sum(ind)) %>% ungroup()
dataRep <- dataRep%>% mutate(sum = 1) %>% group_by(groupID1) %>% summarise(sum = sum(sum)) %>% ungroup()
...
data <- data %>% mutate(treatedAny = ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > firstEffect, 1, 0), 
                        ...
                        treatedCompleteCharge = ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > firstEffect & completeBagBan == 1& paperBagCharge == 1, 1, 0), 
                        treatedCompleteNoCharge = ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > firstEffect & completeBagBan == 1& paperBagCharge == 0, 1, 0), 
                        treatedPartial = ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > firstEffect & partialBagBan == 1, 1, 0), 
                        treatedPartialCharge = ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > firstEffect & partialBagBan == 1 & paperBagCharge == 1, 1, 0),
                        treatedPartialNoCharge = ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > firstEffect & partialBagBan == 1 & paperBagCharge == 0, 1, 0),
                        treatedCharge = ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > firstEffect & plasticBagCharge == 1, 1, 0),
                        treatedChargeCharge = ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > firstEffect & plasticBagCharge == 1 & paperBagCharge == 1, 1, 0),
                        treatedChargeNoCharge = ifelse(as.Date(paste0(year, "-01-01"), format="%Y-%m-%d") > firstEffect & plasticBagCharge == 1 & paperBagCharge == 0, 1, 0), 
...
save(data, file="data/processed/02_data_merged/02_merged_bigcell_year.rda")
save(dataBalanced, file="data/processed/02_data_merged/02_merged_bigcell_year_balanced.rda")
