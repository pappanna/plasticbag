...
cleanup <- cleanup %>% mutate(itemsPP = totalItems / people, 
                              ...
                              percPlasticBag = ifelse(!is.na(totalItems), plasticGroceryBag / totalItems, NA), 
                              percOtherBag = ifelse(!is.na(totalItems), plasticOtherBag / totalItems, NA), 
                              percBag = ifelse(!is.na(totalItems), (plasticGroceryBag + plasticOtherBag) / totalItems, NA),
                              ...
                              plasticBagPP = plasticGroceryBag / people, 
                              plasticBagPPPM = ifelse(miles > 0, plasticGroceryBag / people / miles, NA),
...
cleanup <- cleanup %>% dplyr::select(groupIDCounty, groupIDZip, groupID1, groupID01, groupID001, 
                                     id, zip, county, state, fips, lat, lon, date, year, month, iccd, people, miles, 
                                     totalItems, itemsPP, itemsPPPM, 
                                     plasticGroceryBag, plasticBevBottle, paperBags, 
                                     percPlasticBag, percOtherBag, percBag, percPlasticBottle, percPlasticBottleCap, percPlasticStraws, percPlasticFoodCont, percPaperBag,
                                     plasticBagPP, plasticBagPPPM, 
...
write_csv(cleanup, file="data/processed/00_data_cleanup_locations.csv")
